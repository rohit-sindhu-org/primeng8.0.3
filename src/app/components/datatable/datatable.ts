...
import * as XLSX from 'xlsx-style-cli';
...
<div class="ui-radiobutton-box ui-widget ui-radiobutton-relative ui-state-default" (click)="handleClick($event)"
    ...
    <span class="ui-radiobutton-icon" [ngClass]="{'pi pi-circle':checked}"></span>
</div>
</div>
`
})
...
<div class="ui-chkbox-box ui-widget ui-corner-all ui-state-default" (click)="handleClick($event)"
    ...
    <span class="ui-chkbox-icon ui-c" [ngClass]="{'pi pi-check':checked}"></span>
</div>
</div>
`
})
    ...
    *ngIf="dt.rowGroupMode=='subheader' && (rowIndex === 0||(dt.resolveFieldData(rowData,dt.groupField) !== dt.resolveFieldData(dt.dataToRender[rowIndex - 1], dt.groupField)))"
    ...
    <td [attr.colspan]="columns.length">
        <a href="#" *ngIf="dt.expandableRowGroups" (click)="dt.toggleRowGroup($event,rowData)">
            <span class="pi pi-fw" [ngClass]="{'pi-chevron-circle-down':dt.isRowGroupExpanded(rowData), 'pi-chevron-circle-right': !dt.isRowGroupExpanded(rowData)}"></span>
        </a>
    ...
    <ng-template ngFor let-col [ngForOf]="columns" let-colIndex="index">
        <td #cell *ngIf="!dt.rowGroupMode || (dt.rowGroupMode == 'subheader') ||
            ...
            <a href="#" *ngIf="col.expander" (click)="dt.toggleRow(rowData,$event)">
                <span class="ui-row-toggler pi pi-fw ui-c" [ngClass]="{'pi-chevron-circle-down':dt.isRowExpanded(rowData), 'pi-chevron-circle-right': !dt.isRowExpanded(rowData)}"></span>
            </a>
...
export class ScrollableView implements AfterViewInit, AfterViewChecked, OnDestroy {
    public dt: DataTable;
    constructor( @Inject(forwardRef(() => DataTable)) datatable, public el: ElementRef, public renderer: Renderer) { 
this.dt=datatable as DataTable;
    }
    ...
    ngAfterViewChecked() {
        if (this.virtualScroll && !this.rowHeight) {
            let row = DomHandler.findSingle(this.scrollTable, 'tr.ui-widget-content');
            if (row) {
...
@Component({
    ...
    template: `
<div [ngStyle]="style" [class]="styleClass" [style.width]=""
...
<div class="ui-datatable-loading-content" *ngIf="loading">
    <i class="pi pi-circle-o-notch pi-spin pi-2x"></i>
</div>
...
<span class="pi pi-arrow-down ui-datatable-reorder-indicator-up" style="position: absolute; display: none;"></span>
<span class="pi pi-arrow-up ui-datatable-reorder-indicator-down" style="position: absolute; display: none;"></span>
</div>
`,
    providers: []
})
export class DataTable implements AfterViewChecked, AfterViewInit, AfterContentInit, OnInit, DoCheck, OnDestroy, BlockableUI {
    ...
    @ContentChild(HeaderColumnGroup,{static:false}) headerColumnGroup: HeaderColumnGroup;

    @ContentChild(FooterColumnGroup,{static:false}) footerColumnGroup: FooterColumnGroup;
    ...
    constructor(public el: ElementRef, public differs: IterableDiffers,
        public renderer: Renderer, public changeDetector: ChangeDetectorRef, private ngZone: NgZone) {
        window.onresize = (e) => {
            ngZone.run(() => {
    ...
    sort(event, column: Column) {
        ...
        if (targetNode == 'TH' || (targetNode == 'SPAN' && !DomHandler.hasClass(event.target, 'ui-c'))) {
    ...
    onRowGroupClick(event) {
        ...
        if (this.sortableRowGroup) {
            let targetNode = event.target.nodeName;
            if ((targetNode == 'TD' || (targetNode == 'SPAN' && !DomHandler.hasClass(event.target, 'ui-c')))) {
    ...
    findIndexInSelection(rowData: any) {
        ...
        if (this.selection) {
            for (let i = 0; i < this.selection.length; i++) {
                if (ObjectUtils.equals(rowData, this.selection[i], this.dataKey)) {
                    index = i;
                    break;
                }
            }
    ...
    isSelected(rowData) {
        return ((rowData && ObjectUtils.equals(rowData, this.selection, this.dataKey)) || this.findIndexInSelection(rowData) != -1);
    }
    ...
    switchCellToEditMode(cell: any, column: Column, rowData: any) {
        if (!this.selectionMode && this.editable && column.editable) {
            ...
            if (cell != this.editingCell) {
                ...
                let focusable = DomHandler.findSingle(cell, '.ui-cell-editor input');
                if (focusable) {
    ...
    onColumnResizeEnd(event) {
        ...
        if (columnWidth + delta > parseInt(minWidth)) {
            if (this.columnResizeMode === 'fit') {
                ...
                if (this.scrollable) {
                    this.scrollBarWidth = this.scrollBarWidth || DomHandler.calculateScrollbarWidth();
    ...
    onColumnDragover(event) {
        if (this.reorderableColumns && this.draggedColumn) {
            event.preventDefault();
            let iconWidth = DomHandler.getHiddenElementOuterWidth(this.reorderIndicatorUp);
            let iconHeight = DomHandler.getHiddenElementOuterHeight(this.reorderIndicatorUp);
